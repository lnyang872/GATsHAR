...
def prepare_wide_data_with_energy(stock_folder, energy_folder):
    ...
    energy_files = sorted(glob.glob(os.path.join(energy_folder, "*.csv")))
    energy_dfs = []
    for f in energy_files:
        name = os.path.splitext(os.path.basename(f))[0]
        energy_dfs.append(pd.read_csv(f, header=None, names=[f"ENERGY_{name}"]))
    
    if energy_dfs:
        energy_wide = pd.concat(energy_dfs, axis=1)
        energy_wide = energy_wide.reindex(stock_wide.index).ffill().fillna(0)
        full_wide = pd.concat([stock_wide, energy_wide], axis=1)
    else:
        full_wide = stock_wide
        
    full_wide = full_wide.ffill().bfill().fillna(0)
...
def main():
    ...
    print("Loading and consolidating stock and energy data...")
    wide_df, stock_ids, num_stocks = prepare_wide_data_with_energy(STOCK_FOLDER, ENERGY_FOLDER)
    ...
    print("Begin training the final LSTM model using the optimal parameters....")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), 'best_lstm_energy.pth')
            counter = 0
        else:
            counter += 1
            if counter >= PATIENCE:
    ...
    print("The final model training has been completed, and the optimal weights have been loaded for prediction....")
    model.load_state_dict(torch.load('best_lstm_energy_l.pth'))
    model.eval()
    ...
    print("\n" + "="*25 + " LSTM Model Performance (Macro Average) " + "="*25)
    ...
    output_path = os.path.join(output_dir, 'lstm_energy_tuned_predictions.xlsx')
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        for stock_id, group_df in tqdm(final_results.groupby('stock_code'), desc="write to Sheet"):
