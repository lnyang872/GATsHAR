...
class FinalHeteroDataset(InMemoryDataset):
    """
    A dataset class for constructing sequences of heterogeneous graphs.
    """
    ...
    def __init__(self, hdf5_file_vol, hdf5_file_volvol, stock_har_rv_folder, 
                 ...
                 include_energy=True,      # Energy switch (whether it contains energy-related information; default is true)
                 use_har_features=True):   # HAR Feature Switch (indicates whether HAR features are included; default is true)
        ...
        # --- Set the energy path according to the include_energy setting ---
        ...
        if os.path.exists(self.processed_paths[0]):
            print(f"--- Loading data from the cache directory '{self.processed_dir}'... ---")
    ...
    def _load_features_to_dict(self, folder_path, file_ids):
        ...
        for asset_id in file_ids:
            try:
                ...
                print(f"Warning: The file for asset {asset_id} could not be found in the folder {folder_path}.")
        return feature_dict

    def process(self):
        print("\n==========================================================")
        print("====== Begin executing the process() method ... ======")
        print(f"====== [Configuration] Energy switch (include_energy): {'on' if self.include_energy else 'off'} ======")
        print(f"====== [Configuration] HAR Feature Switch (use_har_features): {'on' if self.use_har_features else 'off'} ======")
        print("==========================================================")
        ...
        if not all_har_features:
            raise FileNotFoundError("No HAR-RV feature files were found in the specified path.")
        ...
        with h5py.File(self.hdf5_file_vol, 'r') as f_vol, h5py.File(self.hdf5_file_volvol, 'r') as f_volvol:
            ...
            for i in tqdm(range(iterator_range), desc="Generating a sequence of heterographic diagrams"):
                ...
                for j in range(self.seq_length):
                    ...
                    for node_idx, node_id in enumerate(self.node_order):
                        ...
                        # === Temporary list for sequentially collecting the features of this node ===
                        features_to_concat = []

                        if node_id in self.stock_ids:
                            # --- A. Stock node ---
                            
                            # Conditional addition of HAR features
                            ...
                            # --- C. Energy node  ---
                            # When `include_energy=False`, we must pad with 0 to maintain HDF5 index alignment.
                            
                            if feature_len_per_step == -1:
                                # Consider the HAR switch
        ...
        if skipped_samples_count > 0:
            print(f"\n[Data Processing Warning] As no edges were detected, {skipped_samples_count} samples have been omitted.")

        if not data_list:
            raise ValueError("No valid pattern samples were successfully created.")

        print("\nThe final dataset is currently being organised and preserved....")
        data, slices = self.collate(data_list)
        torch.save((data, slices), self.processed_paths[0])
        print(f"The dataset cache has been successfully created and saved to {self.processed_paths[0]}")
        print("==========================================================")
