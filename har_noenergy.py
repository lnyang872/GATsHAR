...
def main():
    ...
    print("Constructing the HAR features for all stocks...")
    stock_feats_list = []
    ...
    print("Commencing individual stock training: HAR-AllStocks (stocks only)...")
    ...
    cutoff = int(len(stock_df) * 1.0)  # Use all data
    ...
    for stock_col in tqdm(stock_df.columns, desc="Processing Stocks"):
        ...
        # 3. Merge: [This stock's characteristics + all other stock characteristics]
        X = pd.concat([X_own_stock, X_other_stocks], axis=1)
    ...
    print("\n" + "="*25 + " HAR-AllStocks (stocks only) Model performance (Macro Average) " + "="*25)
    ...
    # --- 4. write to Excel ---
    output_dir = 'results_high'
    os.makedirs(output_dir, exist_ok=True)

    output_path = os.path.join(output_dir, 'har_ridge_allstocks_ONLY_predictions.xlsx')
